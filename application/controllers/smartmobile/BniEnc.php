...
class BniEnc
{
	...
	public static function encrypt_string($string, $cid, $secret) {
		return self::doubleEncrypt(strrev(time()) . '.' . $string, $cid, $secret);
	}

	public static function decrypt_string($hased_string, $cid, $secret) {
		$parsed_string = self::doubleDecrypt($hased_string, $cid, $secret);
		list($timestamp, $data) = array_pad(explode('.', $parsed_string, 2), 2, null);
		if (self::tsDiff(strrev($timestamp)) === true) {
			return $data;
